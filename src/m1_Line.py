...
class Line(object):
    ...
    def line_plus(self, other_line):
        ...
        #        They include the Example in the above doc-string.
        # ---------------------------------------------------------------------
        newstart_x = self.start.x + other_line.start.x
        newstart_y = self.start.y + other_line.start.y
        newend_x = self.end.x + other_line.end.x
        newend_y = self.end.y + other_line.end.y
        line = Line(Point(newstart_x, newstart_y), Point(newend_x, newend_y))
        return line
    ...
    def midpoint(self):
        ...
        #        They include the Example in the above doc-string.
        # ---------------------------------------------------------------------
        x_midpoint = ((self.end.x - self.start.x) / 2) + self.start.x
        y_midpoint = ((self.end.y - self.start.y) / 2) + self.start.y
        point = Point(x_midpoint, y_midpoint)
        return point
    ...
    def reset(self):
        ...
        originalline = Line(self.start, self.end)
        return originalline
